...
#The results are: The algorithm could be more accurate when k = 1. And higher the values of k, the lower the accurate. 
#It is clear that as k increases after 1, the quality of algorithm to correctly classify the observations decreases. knowing more about variables allows us to refine the algorithm and the accuracy could become higher on classifying the test data. However, it seems more variables, the algorithm become more confused on toward difference direction.
...
# Defining by borough to make analysis is pretty convincing classification. 
# However in my understanding this would much more depends on the bias of observer,
# and this is decided by the knowledge of the observer in the data set.
